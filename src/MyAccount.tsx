        ...
        <section className="bg-gradient-to-br from-indigo-100 to-purple-50 rounded-xl p-8 shadow-lg backdrop-blur-sm">
          ...
          {user && (
            <div className="relative group transform hover:scale-[1.01] transition-all duration-300">
              <div className="absolute -inset-1 bg-gradient-to-r rounded-lg blur opacity-25 group-hover:opacity-40 transition duration-300" />
              <div className="relative bg-white/80 p-6 rounded-lg shadow-xl">
                
                {/* Avatar Section */}
                <div className="flex items-center justify-center mb-6 relative">
                  <div className="absolute w-32 h-32" />
                  <div className="w-24 h-24 rounded-full overflow-hidden ring-4 ring-indigo-400 ring-offset-white hover:scale-105 transition-transform duration-300 relative z-10">
                    <img
                      src={user.photoURL || `https://api.dicebear.com/6.x/avataaars/svg?seed=${user.gender.toLowerCase() === 'male' ? 'Max' : user.gender.toLowerCase() === 'female' ? 'Anna' : 'Sammy'}`}
                      alt="Avatar"
                ...
                <div className="space-y-4">
                  {/* Username */}
                  <div className="flex items-center space-x-4 p-3 hover:bg-indigo-50/80 rounded-lg transition-all duration-300 transform ">
                    ...
                    <div className="flex-grow">
                      ...
                      {isEditing ? (
                        <input
                          ...
                          className={`text-lg font-semibold text-gray-800 bg-transparent border-b-2 ${
                            isEditing ? 'border-indigo-600 bg-indigo-50' : 'border-indigo-300'
                          } focus:outline-none focus:border-indigo-600 transition-all duration-300`}
                        />
                      ) : (
                    ...
                    {!user.isGoogleUser && (
                      isEditing ? (
                        ...
                        <Edit
                          ...
                          className="text-purple-600 cursor-pointer hover:scale-110 transition-transform  hover:rotate-180"
                        />
                      )
                    )}
                  </div>
